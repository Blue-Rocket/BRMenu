...
@implementation BRMenuUIMappingRestKit
	...
	dispatch_once(&onceToken, ^{
		...
		result = [RKBlockValueTransformer valueTransformerWithValidationBlock:^BOOL(__unsafe_unretained Class inputValueClass, __unsafe_unretained Class outputValueClass) {
			...
			if ( [inputValue isKindOfClass:[NSNumber class]] ) {
				UIColor *color = [BRMenuUIStyle colorWithRGBAHexInteger:[inputValue unsignedIntegerValue]];
				...
				*outputValue = @([BRMenuUIStyle rgbaHexIntegerForColor:inputValue]);
			}
			return YES;
		}];
	});
...
+ (RKObjectMapping *)uiStyleMapping {
	RKObjectMapping* mapping = [RKObjectMapping mappingForClass:[BRMenuUIStyle class]];
